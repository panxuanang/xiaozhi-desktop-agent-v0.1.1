...
如果希望 GitHub Release 页面直接出现 EXE，可创建并推送类似 `v0.1.1` 的 tag；工作流会自动把 `XiaoZhiSetup.exe` 附到 Release。


## v0.1.1 构建自检

这版专门修复 Windows GitHub Runner 在 self-check 结束时删除仍被 SQLite 占用的 `tasks.db` 而触发 `WinError 32` 的问题。Task Center 现在会在临时目录清理前显式 checkpoint + close。
